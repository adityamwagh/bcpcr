...
class KITTIPairDataset(PairDataset):
    AUGMENT = None
    DATA_FILES = {"train": "configs/train_kitti.txt", "val": "configs/val_kitti.txt", "test": "configs/test_kitti.txt"}
    ...
    def __init__(
        self, phase="train", transform=None, random_rotation=True, random_scale=False, manual_seed=False, config=None
    ):

        if self.IS_ODOMETRY:

            if self.LOCAL:
                self.root = root = "/home/aditya/dataset"
            else:
                self.root = root = "/scratch/amw9425/dataset"

            random_rotation = self.TEST_RANDOM_ROTATION

        if self.LOCAL:
            self.icp_path = os.path.join("/home/aditya/dataset", "icp_local")
        else:
            self.icp_path = os.path.join("/scratch/amw9425/dataset", "icp_hpc")
    ...
    def get_all_scan_ids(self, drive_id):
        if self.IS_ODOMETRY:
            ...
            fnames = glob.glob(self.root + "/" + self.date + "_drive_%04d_sync/velodyne_points/data/*.bin" % drive_id)
